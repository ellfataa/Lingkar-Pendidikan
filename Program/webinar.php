...
$today = date('Y-m-d');
$query = $conn->prepare("SELECT * FROM webinar WHERE tanggal >= ? ORDER BY tanggal ASC");
...
<body>
    ...
    <div class="container my-5">
        <h1 class="text-center mb-4" style="color: #050c9c;">Webinar Terbaru</h1>
        <div class="row">
            <?php while ($row = $result->fetch_assoc()): ?>
                <div class="col-md-6 mb-4">
                    <div class="card shadow-sm">
                        <?php if (!empty($row['poster'])): ?>
                            <img src="poster/<?= $row['poster'] ?>" class="card-img-top" alt="Poster Webinar">
                        <?php endif; ?>
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($row['judul']) ?></h5>
                            <p class="card-text"><?= nl2br(htmlspecialchars($row['deskripsi'])) ?></p>
                            <p><strong>Tanggal:</strong> <?= date('d M Y', strtotime($row['tanggal'])) ?><br>
                               <strong>Waktu:</strong> <?= date('H:i', strtotime($row['waktu'])) ?></p>
                            <a href="daftar_webinar.php?id=<?= $row['id_webinar'] ?>" class="btn btn-primary" style="background-color: #050c9c;">Daftar Sekarang</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
            <?php if ($result->num_rows == 0): ?>
                <div class="col-12">
                    <div class="alert alert-info text-center">Belum ada webinar yang akan datang.</div>
                </div>
            <?php endif; ?>
        </div>
    </div>
